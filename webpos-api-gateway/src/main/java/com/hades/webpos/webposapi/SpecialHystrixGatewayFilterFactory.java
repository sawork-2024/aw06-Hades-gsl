...
public class SpecialHystrixGatewayFilterFactory
  ...
  public GatewayFilter apply(Config config) {
    return (exchange, chain) -> {
      ...
      //      } else {
      //        command =
      //            new MyRouteHystrixCommand(config.getFallbackUri(), exchange, chain, timeout,
      // path);
      //      }
      ...
      return Mono.create(
              s -> {
                Subscription sub =
                    command.toObservable().subscribe(s::success, s::error, s::success);
